...
import { formatDateInIst, formatTimeInIst, toIstDateTimeLocalValue, toUtcIsoFromIstInput } from "@/lib/ist-time";
...
export default function ClassScheduling() {
    ...
    try {
      ...
      const res = await apiRequest("/classes", {
        ...
        body: JSON.stringify({
          ...
          topic,
          date: toUtcIsoFromIstInput(date),
  ...
  const handleStartSession = async (session: LiveClass) => {
    try {
      if (session.status === "scheduled") {
        await apiRequest(`/classes/${session._id}/status`, {
          method: "PUT",
          body: JSON.stringify({ status: "live" })
        });
      }
      window.location.href = `/teacher/live/${session._id}/zoom`;
    } catch (err: any) {
      alert(err.message || "Error starting live class");
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setIsSubmitting(true);
      const res = await db.user.updateLiveClass(editingClass._id, {
        ...editingClass,
        date: toUtcIsoFromIstInput(editingClass.date),
      });
      if (res.success) {
  ...
  const formatDate = (dateString: string) => {
    return formatDateInIst(dateString, { month: 'short', day: 'numeric', year: 'numeric' });
  };

  const formatTime = (dateString: string) => {
    return formatTimeInIst(dateString);
  };
                   ...
                   {liveClasses.map((session) => (
                     <div key={session._id} className="flex flex-col md:flex-row justify-between items-center p-6 bg-surface_container_lowest rounded-2xl border border-outline_variant/10 shadow-sm hover:shadow-ambient transition-all group">
                        ...
                        <div className="flex items-center gap-4 mt-4 md:mt-0">
                           ...
                           {session.zoomStartUrl || session.zoomLink ? (
                             <button
                               type="button"
                               onClick={() => handleStartSession(session)}
                               className="p-2 bg-surface_container_highest hover:bg-primary/20 hover:text-primary rounded-full transition-colors"
                               title="Start in platform"
                             >
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       ...
                       <input 
                          required
                          type="datetime-local"
                          value={toIstDateTimeLocalValue(editingClass.date)}
