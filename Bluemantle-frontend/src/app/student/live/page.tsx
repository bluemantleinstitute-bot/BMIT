...
import { Clock, Video, AlertCircle, PlayCircle } from "lucide-react";
import Link from "next/link";
import { LiveJoinManager } from "@/components/LiveJoinManager";
import { SessionRefresher } from "@/components/SessionRefresher";
import { db } from "@/lib/db";
import { formatDateTimeInIst, formatTimeInIst } from "@/lib/ist-time";
    ...
    <div className="space-y-8 pb-10">
          ...
          <div className="relative z-10 p-8 flex flex-col md:flex-row gap-8 items-center justify-between">
            <div>
              ...
              <p className="text-on_surface_variant text-lg mb-6">
                Instructor: {liveNow.teacherId?.name || "Assigning..."} · {liveNow.batchId?.name} · {formatTimeInIst(liveNow.date)}
              </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {upcoming.length > 0 ? upcoming.map((c: any, i: number) => (
          <KnowledgeCard key={c._id || i}>
            <div className="p-4 flex justify-between items-center">
              <div>
                <h4 className="font-bold text-on_surface">{c.topic}</h4>
                <p className="text-sm text-on_surface_variant">
                  {formatDateTimeInIst(c.date, { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })} · {c.teacherId?.name || "TBA"}
                </p>
              </div>
