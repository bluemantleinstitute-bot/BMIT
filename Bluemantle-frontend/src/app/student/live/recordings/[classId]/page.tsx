...
export default function LiveRecordingPlayer({ params }: { params: Promise<{ classId: string }> }) {
  ...
  const platformRecordingUrl = `${API_BASE_URL}/classes/${classId}/recording-stream`;
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-black flex flex-col relative">
        <div className="flex-1 flex items-center justify-center relative p-4 md:p-12">
           {isYouTubeRecording ? (
             ...
             <video
               className="w-full max-w-5xl aspect-video rounded-2xl border border-outline_variant/10 bg-black shadow-2xl"
               src={platformRecordingUrl}
               controls
               controlsList="nodownload noremoteplayback"
               disablePictureInPicture
               crossOrigin="use-credentials"
               onPlay={markReplayAttendance}
               onEnded={markReplayAttendance}
             />
           )}
        </div>
